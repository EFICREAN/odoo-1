...
class PartnerGetVat(models.Model):
    _inherit = 'res.partner'

    registration_name = fields.Char(
        string='Nombre',
        select=True,
    )
    ...
    @api.multi
    def vat_change_pe(self, vat):
        if not vat:
            return False
        # ~ validate = self.check_vat_pe(vat)
        type = self.type
        v = self.env['table.pe.02'].browse(type)
        vat_type = v.code
        res = {}
                ...
                for li in texto_consulta:
                    if temp == 1:
                        ...
                        dist_id = ditrict_obj.search(
                            [('name', '=', district), ('province_id', '!=', False), ('state_id', '!=', False)], limit=1)
                        if dist_id:
